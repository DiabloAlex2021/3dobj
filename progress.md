...
- New user request: add "Nocturne of Steel", another Meshy AI generated figure (silver-white bob hair, dark ornate layered dress, gloved arms holding a pistol, tall boots).
- User first supplied a USDZ export; while an agent was mid-conversion via the established USDZ->GLB pipeline, the user instead supplied a direct GLB export (`Meshy_AI_Nocturne_of_Steel_0714231410_texture.glb`) of the same object and asked to use that instead — simpler, and skips the lossy USD conversion step. The in-progress USDZ pipeline was stopped and the direct GLB used as the source.
- First embedding attempt used the direct GLB's textures completely unmodified (4x 4096x4096 PNGs), ballooning `coffee_mug_3d.html` to ~142.4MB — over GitHub's hard 100MB single-file push limit, not just the earlier soft 50MB warning. Caught and fixed: extracted the 4 textures (base color, normal, metallic/roughness, emissive) with `pygltflib`, downscaled/re-encoded to match the established pattern (1536px base color JPEG ~285KB, 1024px normal/ORM/emissive JPEGs ~150-220KB each), rebuilt the GLB keeping mesh/UV/material/accessor structure identical. Source GLB dropped from 51.67MB to 1.15MB (97.8% reduction).
- A first "validation complete" claim on this object turned out to be hollow: the embedded `<script type="text/plain" id="nocturneGlbData">` block had been inserted immediately before the file's *final* `</script>` tag, i.e. inside the main logic script rather than alongside the other `*GlbData` blocks — since the placeholder tag's own literal `</script>` closed the browser's HTML parser early, it silently truncated the rest of the main script (canvas init, render loop, etc.), so the button existed but nothing was actually running. Fixed by moving the data block to the correct location (with the other `*GlbData` script tags, before the main logic `<script>` opens) — confirmed the extracted main script now parses cleanly.
- Added as a new sibling static object following the exact Gale/Midnight pattern: menu button, group/rig/base, embedded base64 GLB (`nocturneGlbData`), loader (`initNocturne`, auto-scales via `Box3.setFromObject` since this is a static non-rigged mesh), object config, visibility switch, `render_game_to_text` payload.
- Validation complete via the local preview server (fresh browser session, independently re-verified after the agent's first hollow validation): page loads with all 15 objects' menu buttons present (including the still-working Orbital Lounge Captain moves sub-menu, confirming the earlier script-truncation bug is actually fixed), clicking "Nocturne of Steel" renders the figure correctly scaled and centered on the display base, no console errors, all objects still switch correctly.
- Follow-up copy fix: the info-panel description and `render_game_to_text` `source` field both said "converted from USDZ" (stale, copy-pasted from the Gale/Midnight pattern before the switch to a direct GLB) — corrected to drop the USDZ claim / say "direct GLB export (Meshy AI)".
- File size: `coffee_mug_3d.html` grew from 67,114,541 bytes (~64.0MB) to 78,668,914 bytes (~78.67MB), an increase of ~4.7MB — comfortably under GitHub's 100MB push limit.
...
- New user request: add "Vita (No Coat)" — a variant of the existing Vita VRM figure with her dress mesh hidden.
- Implementation: the same embedded Vita VRM base64 buffer (`vrmFigureData`, already parsed twice for Anime Figure / Original Figure) is now parsed a third time via `buffer.slice(0)` through the existing `loadFigureInstance` helper, added as a new sibling static object (`vitaBareGroup`/`vitaBareRig`/`vitaBareBase`/`vitaBareState`, own `Vita (No Coat)` menu button, `objectConfigs` entry, visibility switch, `available_objects` entry, `render_game_to_text` payload) with no moves menu, since this is display-only.
- Ported `hideVrmMeshesByMaterial(scene, regex)` from the sibling `3dw_Office_with_avatar/character.html` project and called it as `hideVrmMeshesByMaterial(vrm.scene, /Tops/i)` in the third instance's load callback. Verified the regex against this file's own embedded VRM (not assumed): extracted the embedded base64 to a standalone `.glb`, parsed its JSON chunk directly, and confirmed the dress material is named `F00_002_01_Tops_01_CLOTH` (a single primitive on a multi-primitive "Body.baked" mesh alongside separate `Body_00_SKIN` and `Shoes_01_CLOTH` primitives), so hiding it only removes the dress and leaves the shoes and body mesh intact.
- Checked what's actually visible underneath: extracted and viewed the `F00_002_02_Body_00_SKIN` base color texture directly. It is not bare/nude skin — it's a fitted navy/teal sci-fi bodysuit pattern (glowing cyan seams, a diamond collar cutout, thigh-high leg bands, a triangular open-back cutout) painted directly into the base body texture, i.e. VRoid's standard "under-layer" design. Confirmed visually in the browser: the body mesh is complete and continuous with no gaps, holes, or missing polygons anywhere the dress used to cover; her original platform shoes remain on since the `Shoes` material is untouched.
- Validation complete via the local preview server (`3d-objects-server`, port 8892, launch.json lives in the sibling `mochikick` project): took real screenshots (not just a console-error check) before and after — "Vita (No Coat)" loads with `status: "ready"`, `height_units: 2.3`, `dress_hidden: true` in the `render_game_to_text` payload, and visibly renders the bodysuit-texture body with shoes on, viewed from front and back. Regression-checked Original Figure, Anime Figure, Nocturne of Steel, and Cup by clicking through them afterward — all rendered correctly, no console errors, and the existing figures' code was not touched.
- New user request: give "Vita (No Coat)" the same tall black boots as Anime Figure instead of leaving her original VRoid platform shoes on.
- Implementation: reused the existing `addFigureBoots(vrm, model, rig)` function as-is (no reimplementation) — it already hides any mesh with a `shoes`-matching material name and builds the parametric tall-boot meshes (swooped-back shaft, beveled toe box, sole, ankle strap/buckle) from live ankle/knee bone world positions, then attaches them to the `foot`/`lowerLeg` bones. Added a single new call site in `initFigure()`'s third `loadFigureInstance` callback (the Vita (No Coat) load path): changed the callback signature from `(vrm) =>` to `(vrm, model) =>` so `model` is available, then added `addFigureBoots(vrm, model, vitaBareRig)` right after the existing `hideVrmMeshesByMaterial(vrm.scene, /Tops/i)` call. Anime Figure's own call site (`addFigureBoots(vrm, model, figureRig)`) and the shared boot-geometry builder (`buildFigureBoot`) were not touched.
- Validation complete via the local preview server: "Vita (No Coat)" now renders with tall black boots (dress and original platform shoes both hidden, boots correctly scaled/positioned from her leg bones) — screenshot confirmed. Regression-checked Anime Figure afterward: her boots render identically to before, no console errors, no other gallery object touched.
- File size: `coffee_mug_3d.html` grew from 78,671,385 bytes to 78,671,437 bytes, a 52-byte increase — pure code change, no new embedded assets.
- File size: `coffee_mug_3d.html` grew from 78,668,888 bytes (~78.67MB) to 78,671,385 bytes (~78.67MB), an increase of ~2.5KB (pure JS/markup addition — no new embedded model data, since the third parse reuses the existing Vita VRM buffer).
